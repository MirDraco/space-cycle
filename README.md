...
[스페이스사이클 웹앱 접속]
      ...
      별도 백엔드 회원가입 서버 구축 필요 없음
       │
       ▼ (3. GPS 반경 검증 & 체크인 시작)
      【 Cloud Firestore (구글 클라우드 DB) 】
       체크인 시작 기록 저장
      시간 조작 방지: 폰 시계 대신 구글 서버 시계기록
       │
       ▼ (4. 정해진 시간 머무름)
[체류 완료 판정]
      Firestore의 시작 시각과 현재 구글 서버 시각 비교
       │
       ▼ (5. 보상 획득 & 저장)
      【 Cloud Firestore (유저 데이터 업데이트) 】
